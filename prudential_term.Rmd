...
The data
--------------------
By the nature of the Kaggle completion I got no full description of the features in the trainset. This makes the feature engineering harder or less effective, because I cannot really use much domain knowledge to make strong predictors. 
...
For further more comprehensive statistics is presented in the Appendix.
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Medical_History_15),na.rm = TRUE,binwidth = 7,fill="limegreen") 
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Medical_History_24),na.rm = TRUE,binwidth = 7,fill="limegreen")
```
...
```{r}
ggplot(trainlocal) + geom_bar(aes(x = Product_Info_2),na.rm = TRUE,fill="limegreen" ) 
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Product_Info_4),bins  = 30,fill="limegreen")
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = BMI),bins  = 30,fill="limegreen")
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Ins_Age),bins  = 30,fill="limegreen")
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Family_Hist_2),na.rm = TRUE,bins = 30,fill="limegreen")
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Family_Hist_4),na.rm = TRUE,bins  = 30,fill="limegreen")
...
Init h2o server. As you can see i built a cluster with 2 nodes.
...
modelRF <- h2o.randomForest(x = independentVariables, y = dependentVariable, 
                          ...
                          mtries = -1, ntrees = 100, max_depth = 20, nbins = 200)
})
```
...
modelGBM <- h2o.gbm(x=independentVariables, y=dependentVariable, seed = 1976,training_frame = train, validation_frame=valid,
                 learn_rate=0.035, max_depth=11, nbins=20, ntrees=100, sample_rate=0.6,min_rows=10,stopping_rounds = 3)
})
```
...
modelGrid <- h2o.grid("gbm", x = independentVariables, y = dependentVariable, 
                  ...
                  hyper_params = list(ntrees = 100,
                                      max_depth = c(8),
                                      learn_rate = c(0.015,0.02, 0.025),
                                      nbins = c(20,150),
                                      min_rows = c(3,10),
                                      sample_rate=c(0.6,1),
                                      seed=1976),
...
modelGBMCV <- h2o.gbm(x=independentVariables, y=dependentVariable, seed = 1976,training_frame = train, 
                  learn_rate=0.035, max_depth=11, nbins=20, ntrees=100, sample_rate=0.6,min_rows=10,nfolds = 5,stopping_rounds = 3)
...
ggplot(df) + geom_bar(aes(as.factor(labels),msevalid, fill = as.factor(labels)),stat = "identity") + xlab("Model Type") + ylab("Training MSE") + ggtitle("Training MSE") + scale_fill_discrete(name = "")

ggplot(df) + geom_bar(aes(as.factor(labels),msetrain, fill = as.factor(labels)),stat = "identity") + xlab("Model Type") + ylab("Training MSE") + ggtitle("Validation MSE") + scale_fill_discrete(name = "")

ggplot(df) + geom_bar(aes(as.factor(labels),swq, fill = as.factor(labels)),stat = "identity") + xlab("Model Type") + ylab("Quadratic Weighted Kappa") + ggtitle("Quadratic Weighted Kappa") + scale_fill_discrete(name = "")
...
To build my final ensemble took around 6 hour, so I could not have to much window to play around with the setup. Even so, this was the best performing model I made in this study.
...
h2o.rf.11 <- function(..., ntrees =100, nbins = 20,seed = 1,sample_rate = 0.1) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.12 <- function(..., ntrees =100, nbins = 20,seed = 1,sample_rate = 0.632) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.13 <- function(..., ntrees =100, nbins = 20,seed = 1,sample_rate = 0.9) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.21 <- function(..., ntrees =100, nbins = 100,seed = 1,sample_rate = 0.1) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.22 <- function(..., ntrees =100, nbins = 100,seed = 1,sample_rate = 0.632) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.23 <- function(..., ntrees =100, nbins = 100,seed = 1,sample_rate = 0.9) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.31 <- function(..., ntrees =100, nbins = 200,seed = 1,sample_rate = 0.1) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.32 <- function(..., ntrees =100, nbins = 200,seed = 1,sample_rate = 0.632) 
  h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)
h2o.rf.33 <- function(..., ntrees =100, nbins = 200,seed = 1,sample_rate = 0.9) h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)


h2o.gbm.11 <- function(..., learn_rate=0.02, ntrees =100, nbins=20, sample_rate=0.6, max_depth =9)
  h2o.gbm.wrapper(...,learn_rate=learn_rate, ntrees = ntrees, nbins = nbins, max_depth = max_depth,sample_rate=sample_rate)
h2o.gbm.12 <- function(..., learn_rate=0.035, ntrees =100, nbins=20, sample_rate=0.6, max_depth =9)
  h2o.gbm.wrapper(...,learn_rate=learn_rate, ntrees = ntrees, nbins = nbins, max_depth = max_depth,sample_rate=sample_rate)
h2o.gbm.21 <- function(..., learn_rate=0.02, ntrees =100, nbins=20, sample_rate=0.6, max_depth =11)
  h2o.gbm.wrapper(...,learn_rate=learn_rate, ntrees = ntrees, nbins = nbins, max_depth = max_depth,sample_rate=sample_rate)
h2o.gbm.22 <- function(..., learn_rate=0.035, ntrees =100, nbins=20, sample_rate=0.6, max_depth =11)
  h2o.gbm.wrapper(...,learn_rate=learn_rate, ntrees = ntrees, nbins = nbins, max_depth = max_depth,sample_rate=sample_rate)
...
h2o.deeplearning.2 <- function(..., hidden = c(200,200,200), activation = "Tanh", epochs = 50, seed = 1)  
  h2o.deeplearning.wrapper(..., hidden = hidden, activation = activation, seed = seed)
h2o.deeplearning.3 <- function(..., hidden = c(500,500), activation = "RectifierWithDropout", epochs = 50, seed = 1)
  h2o.deeplearning.wrapper(..., hidden = hidden, activation = activation, seed = seed)
...
```{r results='hide'}
# learner <- c("h2o.rf.11","h2o.rf.12","h2o.rf.13","h2o.rf.21","h2o.rf.22","h2o.rf.23","h2o.rf.31","h2o.rf.32","h2o.rf.33","h2o.gbm.11","h2o.gbm.12","h2o.gbm.21","h2o.gbm.22","h2o.deeplearning.1","h2o.deeplearning.2","h2o.deeplearning.3","h2o.deeplearning.4","h2o.deeplearning.5","h2o.deeplearning.6","h2o.deeplearning.7")
...
learner <- c("h2o.rf.11","h2o.gbm.11")
...
As about the Kaggle result, I submitted 28 models, the first two was realy a wild guess, one had an error, but the rest show the evolution of my models:
...
ggplot(df) + geom_bar(aes(x,scores),stat = "identity", fill="limegreen") + xlab("") + ylab("Score") + ggtitle("Score") + scale_fill_discrete(name = "")
...
My aim to finish in top 25% was achieved by end of the competition, I ranked 668/2715, but then the second test with larger data came and many fall behind with over fitted models, so I end up as 232/2715 which is already top 10%: https://www.kaggle.com/salacika/results

The winner of the competition got 0.67939, my score with the first test was 0.67113 and then 0.67125 with second one (it seems my model was flexible enough, slight over fit maybe). 
...
create_h2o_gbm_wrappers <- function(max_depths = c(8,9,10),learn_rates = c(0.02, 0.03)) {
  ...
  for (i in seq(length(max_depths))) {
    for (j in seq(length(learn_rates))) {
      ...
      body <- sprintf(' <- function(..., learn_rate=%s, ntrees = 2, nbins=20, sample_rate=0.6, max_depth =%s) h2o.gbm.wrapper(...,learn_rate=learn_rate, ntrees = ntrees, nbins = nbins, max_depth = max_depth,sample_rate=sample_rate)',learn_rate, max_depth)
...
create_h2o_rf_wrappers <- function(nbins = c(20,100,200),sample_rates = c(0.1, 0.632,0.9)) {
  ...
  for (i in seq(length(nbins))) {
    for (j in seq(length(sample_rates))) {
      ...
      body <- sprintf(' <- function(..., ntrees =2, nbins = %s,seed = 1,sample_rate = %s) h2o.randomForest.wrapper(..., ntrees = ntrees, nbins = nbins,sample_rate = sample_rate, seed = seed)', nbin,sample_rate)
