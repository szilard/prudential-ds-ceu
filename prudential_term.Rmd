...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Medical_History_15),na.rm = TRUE,binwidth = 7 ) 
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Medical_History_24),na.rm = TRUE,binwidth = 7 )
```
...
```{r}
ggplot(trainlocal) + geom_bar(aes(x = Product_Info_2),na.rm = TRUE ) 
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Product_Info_4),bins  = 30)
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = BMI),bins  = 30)
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Ins_Age),bins  = 30)
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Family_Hist_2),na.rm = TRUE,bins = 30)
```
...
```
```{r}
ggplot(trainlocal) + geom_histogram(aes(x = Family_Hist_4),na.rm = TRUE,bins  = 30)
...
ggplot(df) + geom_bar(aes(x,scores),stat = "identity", fill="orange") + xlab("") + ylab("Score") + ggtitle("Score") + scale_fill_discrete(name = "")



```

These were mostly single GBM with CV, Deep Learning, Ensembles and XgBoost (code: ) 
